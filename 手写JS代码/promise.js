...
class MyPromise {
      ...
      switch (this.PromiseStatus) {
        ...
        case 'resolved':
          try {
            setTimeout(() => {
              resolvePromise(
                promise, successCallback(this.PromiseValue),
                resolve, reject
              )
            }, 0)
          } catch (err) {
        ...
        case 'rejected':
          try {
            setTimeout(() => {
              resolvePromise(promise, failCallback(this.PromiseError), resolve, reject)
            }, 0)
          } catch (err) {
            reject(err)
          }
  ...
  //finally方法，无论成功还是失败，都会执行该方法
    ...
    return this.then((value) => {
      ...
      return MyPromise.resolve(finalCallback()).then(() => { throw err })
    }
    )
  }
  //前置包装函数
